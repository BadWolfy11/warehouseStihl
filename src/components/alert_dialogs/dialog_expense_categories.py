...
class ExpenseDocumentsManager:
    def __init__(self, page: ft.Page, token: str, item_id: int | None = None):
        self.page = page
        self.expense_categories = ExpenseCategories(token)
    ...
    def delete_item(self, e):
        delete_item =self.expense_categories.delete_exp_categories(id=self.item_id)
    ...
    def add(self, e):
        ...
        result = self.expense_categories.new_exp_categories(
    ...
    def load_item(self, item_id : int):
        find_item = self.expense_categories.find_exp_categories(id=item_id)
    ...
    def save(self, e):
        update_expense_documents = self.expense_categories.update_exp_categories(
            name=self.name.value
        )

        if update_expense_documents['status'] == 200:
