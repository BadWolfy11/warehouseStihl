...
from src.components.info_data import fetch_paginated_expense_categories
from src.components.alert_dialogs.dialog_role import RoleManager


class ExpenseCategoriesBar:
    def __init__(self, token: str, page: ft.Page, per_page: int = 10):
        ...
        self.load_expense_documents(self.current_page)


    def previous_page(self, e):
        if self.current_page > 1:
            self.load_expense_documents(self.current_page - 1)

    def next_page(self, e):
        if self.current_page < self.total_pages:
            self.load_expense_documents(self.current_page + 1)

    def load_expense_documents(self, current_page: int):
        result = fetch_paginated_expense_categories(self.token, current_page, self.per_page)
        ...
        if "error" in result:
            ...
            for item in result["items"]:
                edit_item = RoleManager(page=self.page, token=self.token, item_id=item["id"])
